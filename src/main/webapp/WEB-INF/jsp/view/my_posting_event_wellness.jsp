...
<link rel="stylesheet" type="text/css" href="${root }/css/my_posting_event_wellness.css">
<meta charset="UTF-8">
	...
	<div class="mypage_main_wrap">
		...
		<c:if test="${member.member_grade == 3 }">
			<div style="margin-bottom: 15px; font-size: 14px; color: #707070;">
				...
				<span>|</span>
				<span class="myposting_category" onclick="location.href = '${root}/my_posting_event'">이벤트</span>
				<span>|</span>
				<span class="myposting_category" onclick="location.href = '${root}/my_posting'">갤러리</span>
				<span>|</span>
				<span style="font-weight: bold;" class="myposting_category" onclick="location.href = '${root}/my_posting_event_wellness'">웰니스행사</span>
			</div>
		</c:if>
		...
		<table class="posting_list">
        	...
        	<c:forEach items="${mypostingList }" var="myposting_list" varStatus="st">
	        	<tr>
	        		<td>
	        			<input type="checkbox" class="checkbox_manage_posting" name="checkbox_each" value="${myposting_list.event_wellness_idx }">
	        		</td>
	        		<td>${myposting_list.event_wellness_idx }</td>
	        		<td><a href="${root }/event_wellness_detail?no=${myposting_list.event_wellness_idx }">${myposting_list.event_wellness_title }</a></td>
	        		<td>${myposting_list.event_wellness_write_time }</td>
	        		<td>${myposting_list.event_wellness_count }</td>
	        	</tr>
        	</c:forEach>
        </table>
        ...
        // 내 게시글 체크박스 수정
        	function modify_my_posting() {
        		...
        		if($('input[name="checkbox_each"]:checked').length == 1) {
        			...
        			if(confirm_val) {
        				location.href = 'event_wellness_modify?no=' + $('input[name="checkbox_each"]:checked').val()
        			}
        		}
        	}
        </script>
