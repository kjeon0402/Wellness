...
<link rel="stylesheet" type="text/css" href="${root }/css/my_posting.css">
<meta charset="UTF-8">
	...
	<div class="mypage_main_wrap">
		...
		<c:if test="${member.member_grade == 3 }">
			<div style="margin-bottom: 15px; font-size: 14px; color: #707070;">
				...
				<span>|</span>
				<span style="font-weight: bold;" class="myposting_category" onclick="location.href = '${root}/my_posting'">갤러리</span>
				<span>|</span>
		...
		<table class="posting_list">
        	...
        	<c:forEach items="${mypostingList }" var="myposting_list" varStatus="st">
	        	<tr>
	        		<td>
	        			<input type="checkbox" class="checkbox_manage_posting" name="checkbox_each" value="${myposting_list.gallery_idx }">
	        		</td>
	        		<td>${myposting_list.gallery_idx }</td>
	        		<td><a href="${root }/gallery_detail?no=${myposting_list.gallery_idx }">${myposting_list.gallery_title }</a></td>
	        		<c:if test="${member.member_grade == 3 }">
	        		<td>${myposting_list.gallery_writer }</td>
	        		</c:if>
	        		<td>${myposting_list.gallery_write_time }</td>
	        		<td>${myposting_list.gallery_count }</td>
	        	</tr>
        	</c:forEach>
        </table>
        ...
        // 내 게시글 체크박스 수정
        	function modify_my_posting() {
        		...
        		if($('input[name="checkbox_each"]:checked').length == 1) {
        			...
        			if(confirm_val) {
        				location.href = 'gallery_modify?no=' + $('input[name="checkbox_each"]:checked').val()
        			}
        		}
        	}
        </script>
