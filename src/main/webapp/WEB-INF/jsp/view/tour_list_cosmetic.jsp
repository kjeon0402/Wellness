...
<link rel="stylesheet" type="text/css" href="${root }/css/tour_list_cosmetic.css">
<meta charset="UTF-8">
...
<body>
	...
	<div class="tour_list_wrap">
		...
		<div class="cate_wrap">
			<div class="btn_wrap">
				<div><button onclick="location.href='${root}/tour_list_nature'">자연교감</button></div>
				<div><button onclick="location.href='${root}/tour_list_ocean'">오션레져</button></div>
				<div><button style="border: 1px solid #72886C; background-color: #72886C; color: white;" onclick="location.href='${root}/tour_list_cosmetic'">뷰티&헬스,스파</button></div>
		...
		<div class="sort_wrap">
			<div class="sort_span_wrap">
				<span style="font-weight: bold; color: #72886C;" class="s_span" onclick="location.href = '${root}/tour_list_cosmetic'">최신순</span>
				<span class="s_span">|</span>
				<span style="cursor: pointer;" class="s_span" onclick="location.href = '${root}/tour_list_cosmetic_popular'">인기순</span>
				<script>
					function newORpop() {
		...
		<div class="tour_list_wrap">
			<div class="list_wrap">
				<c:forEach items="${cosmeticList }" var="cosmetic_list" varStatus="st">
				<!-- list each -->
				<div class="list_each">
					<div class="each_image_div"><!-- <a href="${root }/tour_detail?category=cosmetic&no=${cosmetic_list.tour_idx }"> --><img src="${root }/tourimg/${not empty cosmeticImgList[st.index].img_path ? cosmeticImgList[st.index].img_path : 'sample_image.svg'}"><!-- </a> --></div>
...
<%-- 						<img src="${root }/img/view/tour/tour_heart_clear.svg" onclick="heartClearOrFull(event)"> --%>
						...
						<c:if test="${not empty member }">
							...
							<c:forEach items="${membersLike }" var="members_like">
								<c:if test="${members_like.wish_small_category == cosmetic_list.tour_idx }">
					...
					<div class="each_content_div">
						<div class="ecd_title"><span class="ecd_title2">${cosmetic_list.tour_name }</span><a href="${root }/tour_detail?category=cosmetic&no=${cosmetic_list.tour_idx }"><button>자세히보기</button></a></div>
						<c:if test="${not empty cosmetic_list.tour_address }">
						<div class="ecd_address">주소 : ${cosmetic_list.tour_address } ${cosmetic_list.tour_address_detail }</div>
						</c:if>
						<c:if test="${not empty cosmetic_list.tour_tel }">
						<div class="ecd_tel">연락처 : ${cosmetic_list.tour_tel }</div>
						</c:if>
						<c:if test="${not empty cosmetic_list.tour_operation1 }">
						<div class="ecd_time1">운영 및 일시 시간 : ${cosmetic_list.tour_operation1 }</div>
						<div class="ecd_time2">※ 상세페이지 참고</div>
						</c:if>
<!-- 						<div class="ecd_time2">09:00~17:00(10월~2월)/입장마감시간 : 16:30</div> -->
						<c:if test="${not empty cosmetic_list.tour_fee1 }">
						<div class="ecd_cost1">이용요금 : ${cosmetic_list.tour_fee1 }</div>
						<div class="ecd_cost2">※ 해당하는 자는 신분증 또는 증빙자료 지참 / ※ 상세페이지 참고</div>
						</c:if>
<%-- 						<div class="ecd_btn"><a href="${root }/tour_detail?category=cosmetic&no=${cosmetic_list.tour_idx }"><button>자세히보기</button></a></div> --%>
					</div>
				</div>
				</c:forEach>
				<script>
					const ecdTitle = document.querySelectorAll('.ecd_title2')
					for(let i = 0; i < ecdTitle.length; i++) {
						if(ecdTitle[i].innerText.indexOf('(') != -1) {
							ecdTitle[i].innerText = ecdTitle[i].innerText.substring(0, ecdTitle[i].innerText.indexOf('('))
						}
					}
				</script>
<!-- 				<div class="list_each"> -->
