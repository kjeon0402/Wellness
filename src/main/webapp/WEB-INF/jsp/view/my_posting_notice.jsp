...
<link rel="stylesheet" type="text/css" href="${root }/css/my_posting_notice.css">
<meta charset="UTF-8">
	...
	<div class="mypage_main_wrap">
		...
		<c:if test="${member.member_grade == 3 }">
			<div style="margin-bottom: 15px; font-size: 14px; color: #707070;">
				<span style="font-weight: bold;" class="myposting_category" onclick="location.href = '${root}/my_posting_notice'">공지사항</span>
				<span>|</span>
		...
		<table class="posting_list">
        	...
        	<c:forEach items="${mypostingList }" var="myposting_list" varStatus="st">
	        	<tr>
	        		<td>
	        			<input type="checkbox" class="checkbox_manage_posting" name="checkbox_each" value="${myposting_list.notice_idx }">
	        		</td>
	        		<td>${myposting_list.notice_idx }</td>
	        		<td><a href="${root }/notice_detail?no=${myposting_list.notice_idx }">${myposting_list.notice_title }</a></td>
	        		<td>${myposting_list.notice_write_time }</td>
	        		<td>${myposting_list.notice_count }</td>
	        	</tr>
        	</c:forEach>
        </table>
        	...
        	function delete_my_posting() {
        		...
        		if($('input[name="checkbox_each"]:checked').length != 0) {
	        		...
	        		if(confirm_val) {
	        			...
	        			$.ajax({
	        				url : 'delete_my_posting_notice',
	        				type : 'POST',
	        				data : list,
	        				success : function(data) {
	        					if($.trim(data) == 0) {
	        						alert('글이 삭제되었습니다.');
	        						location.href = 'my_posting_notice';
	        					} else {
	        						alert('삭제할 수 없습니다.');
	        					}
	        				},
